...
class RocketLander(gym.Env):
    ...
    def __init__(self, settings):
        ...
        self.time_step = 0      # step for making the Landing barge moving 
        self.episode_step_counter = 0 # <-- ADD THIS
    ...
    def _reset(self):
        ...
        if initial_coordinates is not None:
            ...
            # --- MODIFY THIS BLOCK ---
            # Widen the random starting range from 0.1 to 0.4
            # x = W / 2 + np.random.uniform(-random_x_range * W, random_x_range * W)
            x = W / 2 + np.random.uniform(-0.3, 0.3)
    ...
    # Add this new method anywhere inside the RocketLander class

    def _move_barge_sinusoidally(self):
        """Moves the barge left and right in a smooth sine wave pattern."""
        self.time_step += 1
        # Adjust frequency and amplitude to make it easier or harder
        frequency = 0.01 
        amplitude = 0.2 * W # Moves 20% of the screen width in each direction
    
        # Calculate the target x-position for the center of the barge
        target_center_x = (W / 2) + amplitude * math.sin(frequency * self.time_step)
        # Get the current center x-position
        current_center_x = (self.landing_barge_coordinates[1][0] + self.landing_barge_coordinates[0][0]) / 2
        # Calculate the movement needed to reach the target
        x_movement = target_center_x - current_center_x
        # Use the existing move_barge function to apply the change
        self.move_barge(x_movement, left_height=0, right_height=0)


    def _step(self, action):
        self.episode_step_counter += 1 # <-- ADD THIS AT THE VERY TOP
        ...
        # --- ADD THIS BLOCK FOR WIND ---
        # wind_force = self.np_random.uniform(-WIND_POWER, WIND_POWER) / FPS
        # self.lander.ApplyForceToCenter((wind_force, 0), True)
        # -------------------------------
        ...
        # self.move_barge_sinusoidally()
        self._move_barge_sinusoidally() # <-- ADD THIS LINE
        ...
        # --- NEW TERMINATION-ONLY LOGIC ---
        done = False
    
        # First, check for a successful landing anywhere on the barge
        if state[LEFT_GROUND_CONTACT] or state[RIGHT_GROUND_CONTACT]:
            # Define the conditions for a successful landing
            is_upright = abs(state[THETA]) < 0.1  # Rocket is nearly vertical
            is_stable = np.sqrt(np.square(state[X_DOT]) + np.square(state[Y_DOT])) < 0.2 # Very low total speed

            # If all success conditions are met, end the episode
            if is_upright and is_stable:
                done = True
    
        # If not successful, check for failure conditions
        if not done:
            failure_conditions = [
                self.game_over,
                abs(state[XX]) >= 1.0,
                state[YY] < 0 or state[YY] > 1.3,
                abs(state[THETA]) > THETA_LIMIT
            ]
    ...
    def render(self, mode='human', close=False):
        ...
        # Call the individual drawing functions
        self._render_environment()
        self._render_lander()

        # This is the crucial final command that actually draws to the screen
    ...
    def __compute_rewards(self, state, main_engine_power, side_engine_power, part_angle):
        ...
        # GET THE NEW VARIABLES YOU NEED HERE
        current_height = self.untransformed_state[YY]
        timesteps_completed = self.episode_step_counter

        reward = 0
        shaping = -200 * np.sqrt(np.square(state[0]) + np.square(state[1])) \
                  - 75 * np.sqrt(np.square(state[2]) + np.square(state[3])) \
                  - 1000 * abs(state[4]) - 30 * abs(state[5]) \
                  + 32 * state[6] + 20 * state[7]
        ...
        precision_reward = 0
        # Check if either leg has touched down
        if state[LEFT_GROUND_CONTACT] or state[RIGHT_GROUND_CONTACT]:
            rocket_x = self.untransformed_state[XX]
            pad_left_boundary = self.landing_pad_coordinates[0]
            pad_right_boundary = self.landing_pad_coordinates[1]

            # If the rocket's center is between the flags, give a big bonus!
            if rocket_x < pad_left_boundary or rocket_x > pad_right_boundary:
                precision_reward = -20  # Bonus for precision landing


        if current_height < H/2:
            living_penalty = -0.6
        elif current_height < H/3:
            living_penalty = -1
        else:
            living_penalty = -0.25
        ...
        if self.prev_shaping is not None:
            # reward = shaping - self.prev_shaping + precision_reward + living_penalty
            reward = shaping - self.prev_shaping + living_penalty + precision_reward
        self.prev_shaping = shaping

        # penalize the use of engines
        reward += -main_engine_power * 0.4
